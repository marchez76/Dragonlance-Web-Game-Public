...
DECISIONI = [
    ...
    Decisione(
        ...
        'GENERALIZZATA il 03/09/2026, quando lo stesso difetto e\' ricomparso '
        'su `bonus_colpire`: questa non e\' piu\' una regola sulle CD ma la '
        '**prima applicazione** della '
        'decisione 54 (`origine-e-un-dato`), che vale per ogni valore che '
        'possa essere sia letto sia calcolato. Il campo e la sua forma non '
        'cambiano; cambia che non e\' un caso singolo.'),
    Decisione(
    ...
    Decisione(
        54, 'origine-e-un-dato',
        'L\'origine di un valore e\' un dato, non una deduzione',
        'PRINCIPIO. Quando un valore puo\' essere **sia letto dalla fonte '
        'sia calcolato dal sistema**, la sua **origine va dichiarata in un '
        'campo**. Nessun controllo puo\' dedurla dai numeri, perche\' '
        'quando le due strade coincidono il valore letto e quello calcolato '
        'sono **indistinguibili** — e coincidono quasi sempre, che e\' '
        'esattamente cio\' che rende il difetto invisibile. Un valore che '
        'non dichiara la propria origine non e\' verificabile: oggi puo\' '
        'essere giusto e domani sfasarsi senza che nessuno se ne accorga. '
        'DUE APPLICAZIONI, NON DUE DECISIONI. La '
        'decisione 50 (`cd-origine-dichiarata`) e\' la prima e riguarda le '
        'CD (`cd_origine` con `cd_derivazione`); `bonus_origine` con '
        '`bonus_derivazione` sul `bonus_colpire` di `effetto.attacco` e\' la '
        'seconda. Stesso enum — `fonte` | `derivata` | `stimata` — stessa '
        'coppia di campi, stessa forma di controllo: una `derivata` deve '
        'tornare col conto e il controllo lo verifica, una `fonte` e una '
        '`stimata` no ma devono dire da dove vengono. La regola e\' stata '
        'generalizzata al **secondo** caso e non al terzo: e\' successo due '
        'volte in due giri, su campi diversi, e la seconda l\'ha trovata il '
        'committente. '
        'LA DERIVABILITA\' RESTA CALCOLATA, l\'origine no. E\' la stessa '
        'distinzione della decisione 50 (`cd-origine-dichiarata`): si '
        'dichiara cio\' che non si puo\' '
        'ricavare, e si ricava tutto il resto. Un campo `derivabile` '
        'sarebbe un derivato scritto a mano (CLAUDE.md 3). '
        'LA MISURA, al 03/09/2026, e\' cio\' che regge il principio invece '
        'di un\'argomentazione. Dei **94 bonus di attacco** che il bestiario '
        'scrive in prosa **85 tornano col conto** competenza + '
        'caratteristica e 9 no; delle 35 CD in prosa 29 tornano e 6 no. '
        'Gli 85 e i 29 non sono conferme: il bonus della Spada corta del '
        'Baaz e\' **stampato** nel blocco ufficiale SotDQ **e** torna col '
        'conto, come la sua CD 11, e nessuno dei due controlli poteva '
        'vederlo. I due attacchi oggi strutturati sono uno `fonte` (Baaz) e '
        'uno `derivata` (Traag) e si scrivono allo stesso modo: e\' la '
        'dimostrazione su un caso vero, non un\'ipotesi. '
        'I NOVE FUORI CONTO SONO IL GRUPPO CHE INSEGNA, e non sono nove '
        'errori. **Due** portano un addendo che la fonte dichiara e che la '
        'formula non ha un posto dove mettere — il +3 innato del Cavaliere '
        'della Morte e dello Scheletro Guerriero, che sono la stessa voce in '
        'due varianti. **Tre** sono l\'arco lungo dei centauri, dove la 2e '
        'attribuisce alla specie un bonus con gli archi: un tratto di razza '
        'convertito, non uno scarto. **Quattro** — l\'Orso Glaciale e lo '
        'Skyfisher, due attacchi ciascuno — non hanno nessuna nota che li '
        'spieghi. La lezione e\' che il conto non fallisce dove il dato e\' '
        'sbagliato: fallisce dove la **formula e\' incompleta**, e le due '
        'cose si scrivono uguali finche\' l\'origine non e\' un campo. '
        'QUANTI ALTRI CAMPI HANNO QUESTA FORMA — misurato, non stimato, ed '
        'e\' il motivo per cui la decisione e\' generale. Ancora scoperti: '
        'il **bonus di danno** in prosa (110 casi, 96 tornano col conto), i '
        '**bonus di abilita\'** (18, 16 tornano), la **percezione passiva** '
        '(52, 52 tornano). Quest\'ultima e\' il caso limite che spiega il '
        'principio meglio di ogni altro: il conto torna il **cento per '
        'cento** delle volte, e proprio per questo di nessuna si sa se sia '
        'stata letta o calcolata. Un campo dove il conto torna sempre e\' il '
        'posto **peggiore** in cui fidarsi del conto. '
        'IL PRECEDENTE INCONSAPEVOLE, che e\' la scoperta piu\' utile del '
        'giro: `armor_class`, `hit_points` e `challenge_rating` **dichiarano '
        'gia\'** la propria origine, sotto un altro nome — `conversion_status` '
        'piu\' `source`. Il progetto aveva inventato questo campo **tre '
        'volte** senza accorgersi che era lo stesso campo, e `cd_origine` '
        'era la quarta. Unificare i tre nomi e\' un rinominare che tocca 52 '
        'schede e uno schema gia\' committato: **non fatto ora, dichiarato '
        'aperto**, e il momento giusto sara\' quando il primo dei campi '
        'ancora scoperti dovra\' portare l\'origine davvero. '
        'DOVE IL DIFETTO NON PUO\' ESISTERE. Un campo che porta solo '
        '**ingressi** non ha questa forma: `saving_throws` dichiara quali '
        'competenze il portatore ha e non il numero che ne segue, quindi non '
        'c\'e\' niente da confondere. E\' la stessa forma che la '
        'decisione 52 (`attacco-unica-lettura`) ha imposto al personaggio, e '
        'la regola che ne segue e\' che **un campo che si puo\' non '
        'scrivere e\' meglio di un campo la cui origine si deve '
        'dichiarare**: dichiarare l\'origine e\' il rimedio dove il valore '
        'deve stare scritto, non il primo posto dove guardare.'),

    Decisione(
        55, 'origine-sede-unica',
        'Un valore e la sua origine viaggiano insieme, e il vocabolario ha una sede sola',
        'IL DIFETTO. Il progetto aveva inventato il campo "origine" **quattro '
        'volte** senza accorgersene: `conversion_status` piu\' `source` su '
        '`armor_class`, su `hit_points` e su `challenge_rating` — 156 '
        'dichiarazioni gia\' in opera su 52 schede — e poi `cd_origine` con '
        '`cd_derivazione`, e poi `bonus_origine` con `bonus_derivazione`. '
        'Decima struttura doppia, e in una forma nuova: non due file che '
        'divergono, ma **lo stesso concetto con quattro nomi**. La '
        'decisione 54 (`origine-e-un-dato`) l\'aveva vista e dichiarata '
        'aperta, rimandando l\'unificazione al momento in cui il primo '
        'campo ancora scoperto avesse dovuto portare l\'origine davvero. '
        'Quel momento e\' arrivato: ogni campo nuovo sarebbe stata la '
        'quinta reinvenzione. '
        'SI ESTENDE, NON SI SOSTITUISCE. `conversion_status` piu\' `source` '
        'reggeva da mesi su tre campi e su 156 dichiarazioni: e\' quello a '
        'restare, ed e\' il motivo per cui il rinominare temuto su 52 '
        'schede **non e\' servito**. La traduzione dell\'enum piu\' '
        'giovane in quello piu\' vecchio e\' `fonte` -> `direct`, '
        '`stimata` -> `adapted`, `derivata` -> **`derived`**, che e\' '
        'l\'unico valore che l\'enum piu\' vecchio non aveva: un valore '
        'derivato non e\' `direct` (nessuno l\'ha stampato) ne\' '
        '`adapted` (nessuno l\'ha scelto), ed e\' l\'unico dei cinque che '
        'un controllo puo\' **rifare**. '
        'LA FORMA E\' UN OGGETTO, E IL PREFISSO ERA IL DIFETTO. Un valore e '
        'la sua origine stanno nello **stesso oggetto** — `{value, '
        'conversion_status, source, note}` — che e\' esattamente cio\' che '
        '`armor_class` gia\' era. La forma piatta a prefisso (`cd_origine`, '
        '`bonus_origine`) non scala e **e\' il meccanismo** con cui il '
        'campo si e\' reinventato quattro volte: `attacco` porta due numeri '
        'che hanno ciascuno un\'origine, `bonus_colpire` e `danno[].bonus`, '
        'e ogni numero nuovo pretendeva un prefisso nuovo. Con '
        'l\'incapsulamento l\'origine **non puo\' piu\' mancare** — non '
        'perche\' un controllo la pretende, ma perche\' non c\'e\' un '
        'posto dove scrivere il valore senza di essa. La clausola '
        'condizionale che obbligava il campo di origine e\' stata '
        'cancellata: la garanzia e\' diventata strutturale. '
        'LA SEDE. `conversion_status` e `provenienza` erano definiti **tre '
        'volte** in `$defs` (mostro, oggetto, modello) e la copia di '
        '`oggetto` era **gia\' divergente** (sette voci contro cinque): la '
        'struttura doppia aveva gia\' cominciato a sfasarsi senza che '
        'nessun dato la denunciasse, perche\' un enum ricopiato valida '
        'benissimo finche\' le copie coincidono. Ora i due vocabolari '
        'stanno in `vocabolari.schema.json` — la stessa sede della '
        'decisione 49 (`vocabolario-italiano`) — e i quattro schemi che li '
        'usano li riferiscono con un `$ref` fra file. L\'elenco delle '
        'provenienze e\' l\'**unione** delle tre copie: la fusione allarga '
        'il vincolo dei singoli schemi, ed e\' il prezzo dichiarato di una '
        'sede sola. Unico valore nuovo, `regola di sistema`, che accompagna '
        'sempre un `derived`. '
        'IL CONTROLLO SI METTE ALLA PROVA. Questo difetto non si vede dai '
        'dati e si vede solo guardando gli schemi, e su un repository pulito '
        'un rilevatore rotto e uno funzionante tacciono uguale: '
        '`prova_di_se_stesso()` gli mette davanti tre difetti piantati (un '
        'enum ridigitato fuori sede, un `$defs` condiviso che non e\' un '
        '`$ref`, un `conversion_status` senza `source` accanto) e due '
        'somiglianze legittime che non deve segnalare. E\' lo stesso '
        'schema di `COPIE_PIANTATE` in `_sistema.py`. '
        'COSA NON SI E\' LASCIATO UNIFICARE, dichiarato e non nascosto. '
        '(a) `conversion_status` **e un nome per due concetti**: a livello '
        'di scheda vale "a che punto e\' questa scheda" ({da_compilare, '
        'in_corso, compilato} nei mostri, negli oggetti, nei modelli e nelle '
        'razze; {clonato, in_sospeso} nelle classi), a livello di elemento '
        'dichiara l\'origine di un valore. Rinominare tocca ~166 file piu\' '
        'i generatori `build_*.py`: e\' un giro suo, misurato e non fatto '
        'qui. (b) In razze e classi `source` e\' una **stringa libera** che '
        'porta la citazione di pagina, non un enum: li\' porta piu\' '
        'informazione, ed e\' la ragione per cui resiste alla fusione '
        'diretta. (c) `razza.schema.json` e `classe.schema.json` quasi non '
        'vincolano `mechanics_5e`: l\'origine dichiarata in 105 tratti '
        'razziali e 43 privilegi di classe **non e\' validata da nessuno**. '
        'Zona morta di schema, aperta. (d) Restano scoperti '
        '`danno[].bonus` (110 casi), `skills[].bonus` (18) e '
        '`passive_perception` (52 su 52 che tornano col conto, il caso che '
        'insegna): la forma ora esiste, applicarla e\' lavoro di dati.'),

    Decisione(
        56, 'personaggio-additivo',
        'Il Personaggio si progetta perche\' il background si possa aggiungere dopo',
        'IL RINVIO CONFERMATO. Il Barbaro resta una **razza** e non diventa '
        'un background: tappo, non conversione. Ma delle tre decisioni '
        'sospese e\' l\'unica che tocca la **forma** dello schema e non il '
        'contenuto, e una forma sbagliata oggi si paga riscrivendo domani. '
        'LA PROVVISIONE. Lo schema Personaggio si progetta in modo che '
        'aggiungere un campo `background` piu\' avanti sia **additivo**: '
        'nessun campo esistente cambia significato, nessun dato gia\' '
        'scritto va riletto. In concreto: l\'origine narrativa del '
        'personaggio non si deduce dalla razza ne\' si incastra dentro di '
        'essa, e i tratti che oggi arrivano dalla razza non presuppongono '
        'nella loro forma che la razza sia la loro **unica** sorgente. Il '
        'campo **non si aggiunge ora** — si evita solo di precluderlo. '
        'PERCHE\' NON BASTA DIRLO A VOCE. E\' la stessa lezione delle dieci '
        'strutture doppie: un vincolo che vive solo nella testa di chi '
        'scrive non sopravvive alla sessione in cui e\' stato pensato.'),

    Decisione(
        57, 'fetta-dalla-creazione',
        'Il limite della fetta verticale: la prossima parte dalla creazione, non dal combattimento',
        'IL LIMITE, INDIVIDUATO DAL METODO STESSO. L\'arena misura cosa '
        'manca al motore **a un turno**, e il personaggio le arriva **gia\' '
        'costruito**. Ne segue che nessuna lacuna della **creazione** puo\' '
        'comparire in quella misura, per quanto la si affini: la fetta '
        'verticale non vede cio\' che sta a monte del suo ingresso. Non e\' '
        'una critica al metodo — e\' il metodo che ha reso visibile il '
        'proprio limite, che e\' il motivo per cui va **registrato** invece '
        'che ricordato. '
        'LA CONSEGUENZA. La **prossima fetta parte dalla creazione**, non '
        'dal combattimento. La prova che il limite era reale c\'e\' gia\': '
        '`allowed_classes` non e\' un campo che manca, e\' una decisione '
    ...
    Decisione(
        60, 'metodo-non-filtra',
        "Il metodo di generazione segnala, non filtra il roster",
        "LE DUE FORME. **A**: il metodo si sceglie per primo, e il roster "
        "mostra solo cio' che quel metodo sa produrre. **B**: razza e classe "
        "si scelgono per prime, il metodo viene dopo o e' gia' fissato dal "
        "tavolo, e quando la coppia scelta non e' raggiungibile con quel "
        "metodo il sistema **segnala e propone il tiro**. Si adotta B. "
        "L'ARGOMENTO CHE DECIDE E' IL PRIMO: adottare A vuol dire RIAPRIRE la "
        "decisione 8 (`generazione-caratteristiche`), non applicarla. La 8 "
        "dice «segnala e propone il tiro senza bloccare», e A blocca per "
        "costruzione — non blocca in casi rari, blocca sempre: toglie la voce "
        "dall'elenco prima che la domanda sia posta, e un divieto che "
        "cancella l'opzione e' piu' duro di uno che la nega, perche' non si "
        "vede. Una decisione presa si applica o si riapre in chiaro; non si "
        "annulla per via d'interfaccia. "
        "L'ARGOMENTO PIU' FORTE SUL LUNGO PERIODO E' IL TERZO: A mette una "
        "REGOLA DI CONTABILITA' PRIMA DI UNA DOMANDA D'IDENTITA'. Razza e "
        "classe si scelgono come personaggio — chi sei, da dove vieni, cosa "
        "fai; il metodo di generazione e' convenzione di tavolo, cioe' come "
        "il gruppo ha deciso di tirare. Chiedere il secondo per primo fa "
        "dipendere l'identita' dalla contabilita', ed e' l'ordine sbagliato "
        "anche quando i numeri tornano. "
        "LA MISURA di cosa A avrebbe cancellato in silenzio: 16 coppie su "
        "122, tutte in 2 classi — `cavaliere` (10 su 14) e `paladino`, dove "
        "e' la classe intera. Con A, un tavolo che sceglie l'acquisto a punti "
        "non vedrebbe piu' il paladino, e non gli verrebbe detto perche'. "
        "LA NOTA FINALE, che e' anche la ragione per cui la scelta non "
        "tornera' a porsi: **se il metodo e' impostazione di tavolo, A e B "
        "collassano su B**. Dove il metodo lo fissa il DM per tutti non c'e' "
        "nessuna schermata in cui il giocatore lo sceglie per primo, e "
        "l'unica cosa che il sistema puo' fare e' segnalare. "
        "SEDE. `motore/generazione.percorsi()` mette i tre filtri in fila e "
        "`precluse_dal_metodo()` isola il terzo come AVVISO; "
        "`classi_per_metodo()` resta come misura — la stessa risoluzione "
        "letta per colonna — e non come filtro a monte."),
    Decisione(
        61, 'allineamento-insieme',
        "L'allineamento e' un insieme di nove valori, e due restrizioni sono regole",
        "IL VOCABOLARIO E' CHIUSO E COMPLETO: i nove della 5e, in italiano, "
        "in `vocabolari.schema.json#/$defs/allineamento`. Non si allarga a "
        "consumo come i qualificatori di danno, perche' la fonte lo stampa "
        "intero e ne dichiara la costruzione a due assi. Una restrizione di "
        "classe e' percio' un SOTTOINSIEME dei nove, non una frase, e "
        "l'insieme si deriva dagli assi invece di essere elencato a mano. La "
        "lettura — quale insieme corrisponde a quale frase della fonte, con "
        "la ragione per riga — sta in `dati/_allineamenti.py`; il testo della "
        "fonte resta accanto al risultato come invariante a due sensi, cosi' "
        "che una riformulazione della fonte faccia rifare la lettura invece "
        "di passarci sotto. "
        "IL DIFETTO CHE CHIUDE, misurato e non ricordato: dieci classi su "
        "venti dichiaravano `applied: true` accanto a una stringa di prosa in "
        "due lingue diverse. Un vincolo che si annuncia applicato e non e' "
        "applicabile e' peggio di un campo assente, perche' il campo assente "
        "si vede. "
        "LA VESTE ROSSA sta sull'asse MORALE, tre valori. «Neutrale» e' "
        "l'unica parola del vocabolario che vive su tutti e due gli assi, e "
        "la seconda lettura possibile — «qualunque allineamento che abbia un "
        "neutrale», cinque valori — non e' assurda. A deciderla non e' una "
        "riga della fonte ma la FORMA in cui la fonte presenta le tre Vesti: "
        "una terna simmetrica, Bianca buono, Rossa neutrale, Nera malvagio, "
        "cioe' le tre posizioni dello stesso asse. Leggere la Rossa su un "
        "asse diverso spezzerebbe la simmetria che la fonte costruisce, e "
        "sarebbe una lettura che vale solo per un terzo della terna. "
        "LE DUE VOCI CHE NON SONO VALORI MA REGOLE restano `applied: false` "
        "con la dipendenza dichiarata, ed e' la forma della "
        "decisione 35 (`repertori-sono-filtri`): il filtro delimita, la "
        "scelta avviene dopo. "
        "**Mago dell'Alta Stregoneria**: dipende dalla Veste, che la "
        "decisione 6 (`maghi-delle-torri`) colloca al Test, al 3° livello. "
        "Non si scrive un dato che dipende da una decisione non presa — la "
        "questione aperta (`vesti-livello-sottoclasse`). E l'unione dei tre "
        "insiemi delle Vesti da' tutti e nove: non e' un ripiego per non "
        "decidere, e' cio' che la fonte dice — alla creazione il mago non ha "
        "ancora giurato, e nessun allineamento gli e' precluso. "
        "**Sacerdote degli Ordini Sacri**: il vincolo va sulla DIVINITA', non "
        "sulla classe, ed e' la forma della "
        "decisione 24 (`sfere-sacerdotali`), dove e' il dio scelto a portare "
        "il filtro. Si legge `source_2e.priest_alignment` e non `family`, e "
        "la ragione e' misurata: i due divergono su 7 divinita' su 21, e su 3 "
        "di esse — Chislev, Kiri-Jolith, Mishakal — la famiglia e' piu' larga "
        "del vincolo che la fonte scrive, quindi leggerla perderebbe "
        "informazione vera. Sulle altre 4 e' piu' stretta, e la perdita "
        "sarebbe di segno opposto ma perdita lo stesso. "
        "GLI ALTRI SCRITTORI, cercati il giorno in cui la sede nasce "
        "(CLAUDE.md 3) e non tutti chiusi lo stesso giorno: "
        "`mostri/mechanics_5e.alignment` portava prosa INGLESE dentro lo "
        "strato italiano — lo stesso difetto per cui `_vocabolari.py` e' "
        "nato — e si chiude nel generatore, non a valle; "
        "`mostri/source_2e.alignment` e' strato di fonte e non si tocca mai; "
        "`divinita/family` e `source_2e.priest_alignment` si muovono con la "
        "decisione 24 (`sfere-sacerdotali`). Cio' che resta aperto resta "
        "MISURATO da `_allineamenti.fuori_vocabolario()`: una struttura "
        "doppia misurata non e' chiusa, ma non e' nemmeno invisibile."),
    Decisione(
        62, 'pacchetto-fisso',
        "L'equipaggiamento iniziale e' un pacchetto fisso, non un borsello da spendere",
        "TRE RAGIONI, e la prima e' che il dato lo dichiara gia': 20 classi "
        "su 20 portano `system: \"pacchetto_fisso_5e\"`. Il borsello esiste "
        "per 4 su 20 — le sole classi a cui la fonte 2e da' una formula di "
        "ricchezza — e per le altre 16 la fonte tace e l'SRD non offre "
        "ripiego, perche' «Starting Wealth by Class» non e' fra le sue 45 "
        "sezioni. "
        "LA SECONDA E' IL VERBO: il pacchetto si TRASCRIVE, il borsello va "
        "COMPOSTO. Dei 7 pacchetti che l'SRD stampa ne servono 5, e sono li' "
        "da copiare; gli elenchi delle 5 classi SRD pure. La strada del "
        "borsello chiede invece una decisione editoriale sulla ricchezza di "
        "16 classi, cioe' venti numeri inventati da noi e dichiarati tali "
        "(decisione 7, `doppio-strato`). "
        "LA TERZA E' IL PERIMETRO: il borsello apre il catalogo intero. Chi "
        "compra puo' comprare qualunque riga, quindi le 100 voci SRD che oggi "
        "mancano diventerebbero obbligatorie — il confine «cio' che serve al "
        "combattimento e all'esplorazione» di `_fonti/srd51_equipaggiamento` "
        "non reggerebbe piu'. Col pacchetto ne servono 13. "
        "CONSEGUENZE. Si trascrivono i 5 pacchetti e i 5 elenchi di telaio; "
        "si aggiungono al catalogo le 13 voci nominate e assenti; le 8 classi "
        "senza chassis si COMPONGONO, e la composizione e' una proposta da "
        "portare, non un dato da scrivere di propria iniziativa; le 7 voci "
        "che l'SRD nomina solo dentro la descrizione di un pacchetto, senza "
        "prezzo ne' peso, si guardano nel merito prima di decidere se "
        "diventano oggetti o restano testo del pacchetto; le 5 voci che non "
        "sono oggetti ma SCELTE aperte dentro una famiglia (arma da guerra, "
        "arma da mischia semplice, arma semplice, focus arcano, simbolo "
        "sacro) si registrano come scelte, nella forma della "
        "decisione 35 (`repertori-sono-filtri`): non mancano al catalogo, "
        "mancano all'interfaccia. "
        "COSA NON SI FA. Le 4 formule di ricchezza 2e restano in `source_2e` "
        "come dato di fonte non applicato, con la stessa forma della "
        "decisione 4 (`limiti-di-livello`): il dato si trascrive, la regola "
        "non si applica. "
        "UN DIFETTO SCOPERTO MISURANDO, e che questa strada non usa ma "
        "nemmeno assolve: per 28 oggetti su 79 il prezzo sta dentro una "
        "STRINGA di `mechanics_5e.note` invece che in un campo. E' la stessa "
        "forma della categoria d'arma — un dato che sembra mancante ed e' "
        "solo scartato — e va tirato fuori in un campo, perche' "
        "l'aritmetica della "
